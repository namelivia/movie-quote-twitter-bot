...
video = generate_video_clip(config.GENERAL_CONFIG['videoURI'], quote.start, quote.end)
...
def generate_text_clip(sentence, start, end):
    return TextClip(sentence, fontsize=24, color='yellow',font='Amiri-bold').set_pos('bottom').set_duration(str(end - start))
