...
video = (VideoFileClip(config.GENERAL_CONFIG['videoURI'])
        ...
        .resize(0.3))
text = TextClip(selected.content, fontsize=24, color='yellow', font='Amiri-bold').set_pos('bottom').set_duration(str(selected.end - selected.start))
compo = CompositeVideoClip([video, text])
compo.write_gif(config.GENERAL_CONFIG['outputPath'])
api = twitter.Api(consumer_key=config.GENERAL_CONFIG['consumerKey'],
...
status = api.PostUpdate(selected.content,
        media=config.GENERAL_CONFIG['outputPath'], media_category='tweet_gif')
